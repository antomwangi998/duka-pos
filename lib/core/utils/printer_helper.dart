...
class PrinterHelper {
  ...
  Future<void> printReceiptKenya({
    ...
    bytes += EscPos.lineFeed;
    if (kraPin.isNotEmpty) {
      bytes += _textToBytes('KRA PIN: $kraPin');
      bytes += EscPos.lineFeed;
    }
    ...
    // Items
    bytes += EscPos.alignLeft;
    bytes += _textToBytes('Item            Qty   KSh');
    bytes += EscPos.lineFeed;
    bytes += _textToBytes('--------------------------------');
    bytes += EscPos.lineFeed;
    ...
    for (var item in items) {
      ...
      String qty = '${item['qty']}x';
      String itemTotal = 'KSh${(item['total'] as double).toStringAsFixed(2)}';
      String line = name.padRight(16) + qty.padRight(6) + itemTotal;
      bytes += _textToBytes(line);
      bytes += EscPos.lineFeed;
    }
    ...
    bytes += _textToBytes(
        'Subtotal (excl.VAT): KSh${(subtotal - vatAmount).toStringAsFixed(2)}');
    bytes += EscPos.lineFeed;
    bytes += _textToBytes('VAT ($vatPct): KSh${vatAmount.toStringAsFixed(2)}');
    bytes += EscPos.lineFeed;
    bytes += EscPos.boldOn;
    bytes += _textToBytes('TOTAL: KSh${total.toStringAsFixed(2)}');
    bytes += EscPos.lineFeed;
    bytes += EscPos.boldOff;
    ...
    bytes += EscPos.lineFeed;
    bytes += EscPos.alignLeft;
    final payLabel = paymentMethod == 'mpesa'
        ? 'M-Pesa'
        : paymentMethod == 'credit'
            ? 'Credit'
            : 'Cash';
    bytes += _textToBytes('Payment: $payLabel');
    bytes += EscPos.lineFeed;
    if (mpesaRef != null && mpesaRef.isNotEmpty) {
      bytes += _textToBytes('M-Pesa Ref: $mpesaRef');
      bytes += EscPos.lineFeed;
    }
